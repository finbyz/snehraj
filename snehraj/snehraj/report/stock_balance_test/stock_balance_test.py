...
def execute(filters=None):
	...
	for (company, item, warehouse) in sorted(iwb_map):
		if item_map.get(item):
			...
			if include_uom and report_data['conversion_factor']:
				report_data['no_of_boxes'] = report_data['bal_qty']/report_data['conversion_factor']
...
def get_columns(filters):
	...
	columns = [
		...
		{"label": _("MRP"), "fieldname": "mrp", "fieldtype": "Currency", "width": 90},
		{"label": _("Conversion Factory"), "fieldname": "conversion_factor", "fieldtype": "Float", "width": 100},
		{"label": _("No of Boxes"), "fieldname": "no_of_boxes", "fieldtype": "Float", "width": 100},
	]
...
def get_item_details(items, sle, filters):
	...
	# res = frappe.db.sql("""
	# 	select
	# 		item.name, item.item_name, item.description, item.item_group, item.brand, item.mrp, item.stock_uom %s
	# 	from
	# 		`tabItem` item
	# 		%s
	# 	where
	...
	if items:
		for item in frappe.db.sql("""
			select item.name, item.item_name, item.item_group, item.item_code, item.mrp, item.stock_uom,
		    uom.uom, uom.conversion_factor as conversion_factor
			from `tabItem` item
			left join `tabUOM Conversion Detail` uom on (item.name = uom.parent and uom.uom={0})
			where item.name in ({1}) and ifnull(item.disabled, 0) = 0
			""".format(frappe.db.escape(filters.get("include_uom")),', '.join([frappe.db.escape(i, percent=False) for i in items])), as_dict=1):
				item_details.setdefault(item.name, item)
