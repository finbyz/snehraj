...
def get_wo_details():
	...
	if doc.workstation != data.workstation:
		args.response_data = error_message
		args.has_error = 1
		...
		args.response_data = error_message
		args.has_error = 1
		args.error = "Work Order {} is either not submitted or is cancelled.".format(data.work_order)
		log(args)
		return Response(error_message)

	elif doc.status not in ['Not Started', 'In Process']:
		args.response_data = error_message
		args.has_error = 1
		args.error = "Work Order {} is either Completed or Stopped.".format(data.work_order)
		log(args)
		return Response(error_message)
...
def create_job_card():
	...
	wo = frappe.get_doc("Work Order", data.work_order)

	if wo.workstation != data.workstation:
		args.response_data = error_message
		args.has_error = 1
		args.error = "Workstation {} does not match with Work Order's Workstation.".format(data.workstation)
		log(args)
	...
	elif wo.status not in ['Not Started', 'In Process']:
		args.response_data = error_message
		args.has_error = 1
		args.error = "Work Order {} is either Completed or Stopped.".format(data.work_order)
		log(args)
