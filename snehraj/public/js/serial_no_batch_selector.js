...
erpnext.SerialNoBatchSelector = Class.extend({
		...
		if(this.has_batch) {
			this.values.batches.map((batch, i) => {
				...
				this.map_row_values(row, batch, 'batch_no',
					'selected_qty', 'section', this.values.warehouse);
			});
		} else {
	...
	map_row_values: function(row, values, number, qty_field, section, warehouse) {
				...
				fields: [
					{
						...
						get_query: function() {
							return {
								filters: {item_code: me.item_code, warehouse: me.warehouse_details.name || ''},
								query: 'snehraj.batch_valuation.get_batch_no'
							};
						},
						change: function() {
